...
import { X, ChevronLeft, Check, MinusCircle } from 'lucide-react';

interface QuickRemoveWizardViewProps {
...
export function QuickRemoveWizardView({ collection, onClose, onComplete }: QuickRemoveWizardViewProps) {
  const [stage, setStage] = useState<WizardStage>('countries');
  const [selectedTeam, setSelectedTeam] = useState<Team | null>(null);
  const [removedList, setRemovedList] = useState<string[]>([]);

  // We need to keep track of virtual counts during the session so we don't remove more than available
  const [sessionCounts, setSessionCounts] = useState<{ [id: string]: number }>(() => ({ ...collection.counts }));
  ...
  const handleSelectNumber = (num: number) => {
    ...
    const currentCount = sessionCounts[stickerId] || 0;
    if (currentCount <= 1) return; // Safeguard

    setRemovedList(prev => [stickerId, ...prev]);
    setSessionCounts(prev => ({
      ...prev,
      [stickerId]: prev[stickerId] - 1
    }));
    ...
    <div className="fixed inset-0 z-[100] bg-surface-bright text-on-background flex flex-col select-none overflow-hidden animate-in fade-in duration-200">
      ...
      <header className="flex justify-between items-center px-6 py-4 border-b border-outline-variant/30 bg-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-error uppercase tracking-tighter italic leading-none">
      ...
      <div className="flex-grow overflow-y-auto p-3 pb-24">
        {stage === 'countries' ? (
          ...
          <div className="max-w-xl mx-auto w-full">
            <div className="flex items-center gap-3 mb-4 p-3 bg-error/5 rounded-2xl border border-error/10">
              ...
              <div>
                ...
                <p className="text-[10px] font-label-bold text-on-surface-variant opacity-60">
                  Select sticker number to remove (duplicates only)
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 sm:grid-cols-7 gap-2">
              {Array.from({ length: 20 }, (_, i) => i + 1).map(num => {
                const stickerId = `${selectedTeam?.code}-${num}`;
                const count = sessionCounts[stickerId] || 0;
                ...
                return (
                  <button
                    ...
                    className={`aspect-square border rounded-xl flex items-center justify-center text-lg font-black transition-all shadow-sm ${
                      canRemove 
                        ? 'bg-white border-error text-error hover:bg-error/5 active:scale-90' 
                        : 'bg-surface-container-highest border-outline-variant/30 text-on-surface-variant/20 cursor-not-allowed opacity-30'
                    }`}
                  >
                    {num}
                  </button>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-outline-variant/30 flex flex-col gap-2">
          <div className="flex items-center justify-between px-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant opacity-60">
              Removal Queue ({removedList.length})
            </span>
            <button 
              onClick={() => {
                setRemovedList([]);
                setSessionCounts({ ...collection.counts });
              }}
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            {removedList.slice(0, 15).map((id, i) => (
              <div 
                key={`${id}-${i}`}
                className="flex-shrink-0 bg-error-container text-on-error-container px-4 py-2 rounded-xl text-xs font-black tracking-tighter border border-error/20 animate-in slide-in-from-left-4 duration-300"
              >
                {id}
              </div>
            ))}
